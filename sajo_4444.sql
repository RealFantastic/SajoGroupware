...
ALTER TABLE ELEC_APPROVAL ADD RETURN_RESON VARCHAR2(200) NULL;
ALTER TABLE ELEC_APPROVAL ADD UPDATE_DATE DATE DEFAULT SYSDATE NULL;
COMMENT ON COLUMN ELEC_APPROVAL.FIRST_APPROVER IS '결재시-1';
COMMENT ON COLUMN ELEC_APPROVAL.SECOND_APPROVER IS '결재시-2';
COMMENT ON COLUMN ELEC_APPROVAL.THIRD_APPROVER IS '결재시 -4';
COMMENT ON COLUMN ELEC_APPROVAL.FINAL_APPROVER IS '결재시 -8';
COMMENT ON COLUMN ELEC_APPROVAL.STATUS_CODE IS '0일때 완료, 자기결재= 1,2차결재자 = 3,3차 결재자 =7, 4차결재자=15 ';

ALTER TABLE ELEC_APPROVAL MODIFY RESULT_CODE NUMBER DEFAULT 1;
COMMENT ON COLUMN ELEC_APPROVAL.RESULT_CODE IS '0:접수, 1:회수, 2:진행, 3:반려, 4:완료(STATUS_CODE=0)';
ALTER TABLE ELEC_APPROVAL ADD ISEMERGENCY VARCHAR2(3) DEFAULT 'N' NOT NULL ;

ALTER TABLE HOLIDAY_HISTORY ADD EA_NO VARCHAR2(50) NOT NULL;
ALTER TABLE HOLIDAY_HISTORY MODIFY (HD_CODE VARCHAR2(3));

ALTER TABLE HOLIDAY_HISTORY RENAME COLUMN HD_RESON TO HD_REASON;
ALTER TABLE HOLIDAY_HISTORY MODIFY HD_REASON VARCHAR2(1000);


--전 사원의 정보, 직급명, 부서명 뷰 생성
CREATE OR REPLACE VIEW VIEW_EMP_JOB_DEPT
    AS SELECT * FROM EMPLOYEE LEFT OUTER JOIN JOB USING(JOB_NO) LEFT OUTER JOIN DEPARTMENT USING(DEPT_NO)
;
--전체 결재문서의 정보, 결재양식 뷰 생성
CREATE OR REPLACE VIEW VIEW_ELEC_APPR_FORM_FILE
AS 
SELECT * FROM ELEC_APPROVAL LEFT OUTER JOIN EA_FORM USING(FORM_CODE) LEFT OUTER JOIN APPROVAL_FILE ON ELEC_APPROVAL.EA_NO = APPROVAL_FILE.DOC_NO
;

--전체 결재문서 정보, 결재양식 뷰에서 
CREATE OR REPLACE VIEW VIEW_ELEC_APPR_INFOS
AS 
SELECT 
    A.*
    ,(SELECT EMP_NAME FROM EMPLOYEE WHERE EMP_NO = A.FIRST_APPROVER)  FIRST_NAME
    ,(SELECT EMP_NAME FROM EMPLOYEE WHERE EMP_NO = A.SECOND_APPROVER) SECOND_NAME
    ,(SELECT EMP_NAME FROM EMPLOYEE WHERE EMP_NO = A.THIRD_APPROVER)  THIRD_NAME
    ,(SELECT EMP_NAME FROM EMPLOYEE WHERE EMP_NO = A.FINAL_APPROVER)  FINAL_NAME
    ,(SELECT SIGN_PATH FROM EMPLOYEE WHERE EMP_NO = A.DRAFTER_ID)  DRAFTER_SIGN_PATH
    ,(SELECT SIGN_PATH FROM EMPLOYEE WHERE EMP_NO = A.FIRST_APPROVER)  FIRST_SIGN_PATH
    ,(SELECT SIGN_PATH FROM EMPLOYEE WHERE EMP_NO = A.SECOND_APPROVER) SECOND_SIGN_PATH
    ,(SELECT SIGN_PATH FROM EMPLOYEE WHERE EMP_NO = A.THIRD_APPROVER)  THIRD_SIGN_PATH
    ,(SELECT SIGN_PATH FROM EMPLOYEE WHERE EMP_NO = A.FINAL_APPROVER)  FINAL_SIGN_PATH
    ,(SELECT JOB_NAME FROM VIEW_EMP_JOB_DEPT WHERE EMP_NO = A.DRAFTER_ID)  DRAFTER_JOB_NAME
    ,(SELECT JOB_NAME FROM VIEW_EMP_JOB_DEPT WHERE EMP_NO = A.FIRST_APPROVER)  FIRST_JOB_NAME
    ,(SELECT JOB_NAME FROM VIEW_EMP_JOB_DEPT WHERE EMP_NO = A.SECOND_APPROVER) SECOND_JOB_NAME
    ,(SELECT JOB_NAME FROM VIEW_EMP_JOB_DEPT WHERE EMP_NO = A.THIRD_APPROVER)  THIRD_JOB_NAME
    ,(SELECT JOB_NAME FROM VIEW_EMP_JOB_DEPT WHERE EMP_NO = A.FINAL_APPROVER)  FINAL_JOB_NAME
FROM
VIEW_ELEC_APPR_FORM_FILE A
LEFT OUTER JOIN VIEW_EMP_JOB_DEPT E ON A.DRAFTER_ID = E.EMP_NO
;


ALTER TABLE HOLIDAY_HISTORY ADD LEFT_COUNT NUMBER;

DROP SEQUENCE SEQ_BIRTH_NO;
CREATE SEQUENCE SEQ_BIRTH_NO
START WITH 1
INCREMENT BY 1
;
...
DELETE FROM ELEC_APPROVAL;
DELETE FROM HOLIDAY_HISTORY;
DELETE FROM EMPLOYEE_HOLIDAY;
DELETE FROM EMPLOYEE_HOLIDAY;
DELETE FROM WORK_PIC;
DELETE FROM WORK_BOARD;
DELETE FROM WORK_COMMENT;
DELETE FROM COMPANY;
DELETE FROM PROJECT_PIC;
DELETE FROM PROJ;
